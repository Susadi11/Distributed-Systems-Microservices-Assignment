...
const CheckoutForm = () => {
  ...
  useEffect(() => {
    if (location.state && location.state.total) {
      ...
      console.log("Received total amount:", location.state.total);
    }
  }, [location]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Step 1: Create payment intent with the total amount and required fields
      const { data } = await axios.post(
          "http://localhost:5552/create-payment-intent",
          {
            ...
            currency: "usd",
            paymentMethod: "card",
            // You can pass these if available in your app context
      ...
      if (result.error) {
        ...
        setMessage("✅ Payment successful! Redirecting to checkout page...");

        // Short delay to show the success message before redirecting
        setTimeout(() => {
          // Navigate to checkout page with payment intent ID
          navigate("/checkout", {
            state: {
        ...
        >
          ...
          <div className="flex-1 p-10 shadow-lg bg-white space-y-8">
            ...
            <div className="mb-4 p-4 bg-gray-50 rounded-md">
              <p className="text-lg font-semibold text-[#4a3f55]">Total Amount: ${total.toFixed(2)}</p>
            </div>
            ...
            {message && (
                <div className={`text-center text-sm mt-3 ${message.includes("✅") ? "text-green-600" : "text-red-600"}`}>
                  {message}
                </div>
            )}
          </div>
